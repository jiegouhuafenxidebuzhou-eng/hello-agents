...
class TerminalTool(Tool):
    ...
    def __init__(
        ...
        # 注意：Agent 框架的 Tool 基类用 @property 提供 name/description，
        # 没有 __init__，这里不调用 super().__init__(name=, description=)。
    ...
    def description(self) -> str:
        return (
            "命令行工具 - 执行安全的文件系统、文本处理和代码执行命令"
            "（ls, cat, grep, head, tail等）。用于查看文件内容、搜索代码、统计行数、"
            "分析日志/数据文件等只读操作。"
        )

    @property
    def parameters(self) -> Dict[str, Any]:
        """返回 OpenAI function calling 所需的 JSON Schema 参数描述。"""
        return {
            "type": "object",
            "properties": {
                "command": {
                    ...
                    "description": (
                        "要执行的命令（白名单: " +
                        ', '.join(sorted(list(self.ALLOWED_COMMANDS)[:10])) +
                        "...）。示例: 'ls -la', 'cat file.py', "
                        "'grep TODO *.py', 'head -n 20 data.csv'"
                    ),
                },
            },
            "required": ["command"],
    ...
    def run(self, **kwargs) -> str:
        ...
        ToolRegistry.execute 会以 tool.run(**arguments) 调用，
        因此参数从 kwargs 中按名称取出。
        """
